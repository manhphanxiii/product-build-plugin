...
5. Check whether `<root>/.claude/settings.json` declares `manhphanxiii` under `extraKnownMarketplaces` and enables `build@manhphanxiii`. This is the only thing that makes `/build:*` visible to a cloud session, a second machine, or a teammate; a session invoked locally with the plugin already enabled in its own user settings does not need it, but every other session does. If the file is missing or lacks this declaration, the phase 1 table proposes seeding or patching it from [assets/settings.json.template](assets/settings.json.template).
6. Require Node 22 or newer and working `npx` for visual review.
   ...
   Report whether the setup will use Lavish as the review surface or fall back to the Artifact tool because a local browser cannot be opened.
