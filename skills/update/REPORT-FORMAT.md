...
|---|---|---|---|---|---|
| 2. Concept | /idea-to-product-concept | xong | prd/concept.md | 2026-08-20 | file tồn tại, commit <sha> |
| 3. Prototype | /prototype | đang làm | prd/adr/ | | ADR-0001 nháp |
...
The table is the same seven-step big picture that `/start-repo` creates, minus step 1 (start-repo), which `/start-repo` owns and `/update` never edits, plus one line naming which steps are done and one line naming which are not.
